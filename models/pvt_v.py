...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.sr_ratio > 1:
            ...
            kv = self.kv(x).reshape(B, -1, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k = kv[0]
        x = x.reshape(B, -1, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)[0]
        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ x).transpose(1, 2).reshape(B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
